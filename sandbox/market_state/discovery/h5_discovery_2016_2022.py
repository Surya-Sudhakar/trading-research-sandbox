"""H5 discovery: attach frozen pre-entry market states to frozen Version 0 trades for 2016-2022. Never fits or modifies the strategy/model."""
...
def parse_report_rows(rows):
    ...
    def setting(name):
        ...
        return matches[0]
    for key, value in {'Expert:': 'F1_NY3H_Green_MT5_Version0', 'Symbol:': 'EURUSD',
                       'Period:': 'M15 (2016.01.01 - 2024.12.31)', 'Currency:': 'EUR'}.items():
    ...
    for r in rows[headers[0]+1:]:
        ...
        if timestamp < pd.Timestamp('2016-01-01', tz='UTC') or timestamp >= pd.Timestamp('2025-01-01', tz='UTC') or timestamp < previous or r[1] in seen:
            raise ValueError('invalid/duplicate/out-of-report-window deal')
        previous = timestamp
        seen.add(r[1])
        if r[3] == 'balance':
            ...
            continue
        if r[2] != 'EURUSD' or money(r[5]) != Decimal('.01'):
    ...
    net = balance - initial
    # Reconcile the complete 2016-2024 MT5 ledger first, then freeze H5 discovery to entries before 2023.
    if (len(frame), wins, len(frame)-wins, net) != (5233, 4585, 648, Decimal('-663.51')):
        raise ValueError('expected full-report reconciliation failed')
    if money(setting('Total Net Profit:')) != net or setting('Total Trades:') != 5233 or setting('Total Deals:') != 10466:
        raise ValueError('report summary reconciliation failed')
    discovery = frame.loc[(frame.entry_time >= START) & (frame.entry_time < END)].reset_index(drop=True)
    dwins = int(discovery.outcome.eq('WIN').sum())
    dnet = Decimal(str(round(float(discovery.profit.sum()), 2)))
    if (len(discovery), dwins, len(discovery)-dwins, dnet) != (4162, 3651, 511, Decimal('-504.84')):
        raise ValueError('expected 2016-2022 discovery reconciliation failed')
    return discovery, {'passed': True, 'trades': len(discovery), 'wins': dwins, 'losses': len(discovery)-dwins,
                   'win_rate_percent': 100*dwins/len(discovery), 'net_profit_eur': float(dnet),
                   'history_quality': setting('History Quality:'),
                   'source_report_period': setting('Period:'), 'discovery_period': '2016-01-01 through 2022-12-31 UTC',
...
def run(report=Path('sandbox/test.xlsx'), output=OUTPUT):
    output = Path(output)
    targets = [output/'h5_discovery_2016_2022_trades.csv', output/'h5_discovery_2016_2022_summary.json']
    ...
    summary = {'experiment': 'H5_DISCOVERY_2016_2022_V1',
        ...
        'provenance': {'report_path': str(report), 'report_sha256': before[str(Path(report))],
            ...
            'discovery_feature_rows': len(features), 'eligible_state_observations': len(states),
            'discovery_features_sha256': frame_hash(features), 'assigned_states_sha256': frame_hash(states),
        ...
        'limitations': ['Discovery only: no final H5 conclusion, thresholds or filters may be promoted without later validation.',
            'Frozen model was fitted on 2016-2022, so this entire H5 period is discovery/in-sample for the state model. Feature information at each entry remains causal.',
            'MT5 report states 0% real ticks; execution outcomes are OHLC/backtest discovery evidence only. Exact intrabar execution must be addressed at H8.',
            'Trade report is IC Markets MT5 while the frozen historical feature feed is Dukascopy; cross-feed equivalence is not assumed and must be treated as a limitation.',
